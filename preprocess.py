...
class CICIDS2017Preprocessor:
    ...
    def encode_categorical_features(self, df):
        ...
        if 'Label' in df.columns:
            # Check if the column is string-based (needs encoding)
            if pd.api.types.is_string_dtype(df['Label']):
                print("Label column is string. Performing target encoding...")
                ...
                # If it's already numeric, just ensure it's the correct integer type
                print("Label column is already numeric. Skipping target encoding.")
